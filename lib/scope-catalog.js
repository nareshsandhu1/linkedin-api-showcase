...
const ENTRIES = [
  ...
  // --- Audience Insights API (private/qualified product, gated at app level) ---
  // Permissions doc: "any valid member access token… we recommend using the
  // bare minimum scope permissions like r_ads". So we surface this product
  // alongside Ads & Reporting under the same r_ads scope.
  ['r_ads', 'Audience Insights API', 'Audience size + demographic insights for LinkedIn ad targeting criteria.', [
    { method: 'POST', path: 'https://api.linkedin.com/rest/targetingAudienceInsights?action=audienceInsights', label: 'Targeting Audience Insights' },
  ]],

  // --- Company Intelligence API (private/qualified product) ---
  // Company (target account)-level paid & organic engagement for a Sponsored
  // Account. Requires the Advertising API + Company Intelligence API products,
  // and the member must have consented to r_ads_reporting (no extra scope).
  ['r_ads_reporting', 'Company Intelligence API', 'Company (target account)-level paid & organic engagement, leads, conversions, and firmographics for a Sponsored Account.', [
    { method: 'GET', path: 'https://api.linkedin.com/rest/accountIntelligence?q=account', label: 'Account Intelligence' },
  ]],
...
for (const [scope, product, description, endpoints] of ENTRIES) {
  ...
  catalog[scope].push({ product, description, endpoints });
}
